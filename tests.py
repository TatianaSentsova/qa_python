...
class TestBooksCollector:
    ...
    def test_book_collection_books_genre_true(self):
        assert self.collector.books_genre == {}

    def test_book_collection_favorites_true(self):
        assert self.collector.favorites == []

    @pytest.mark.parametrize('genre', ['Фантастика', 'Ужасы', 'Детективы', 'Мультфильмы', 'Комедии'])
    def test_book_collection_genre_true(self, genre):
        assert genre in self.collector.genre

    @pytest.mark.parametrize('genre_age_rating', ['Ужасы', 'Детективы'])
    def test_book_collection_genre_age_rating_true(self, genre_age_rating):
        assert genre_age_rating in self.collector.genre_age_rating
    ...
    def test_add_new_book_add_without_genre_true(self, name):
        self.collector.add_new_book(name)
    ...
    def test_set_book_genre_for_book_not_from_collector(self):
    ...
    def test_get_books_genre_from_five_books(self, new_books):
        for name, genre in new_books:
            self.collector.add_new_book(name)
            self.collector.set_book_genre(name, genre)
    ...
    def test_get_books_for_children_with_empty_books_genre(self):
        books_for_children = self.collector.get_books_for_children()
    ...
    def test_add_book_in_favorites_for_book_not_from_collector(self):
        assert not self.collector.add_book_in_favorites('Война и мир')

    def test_add_book_in_favorites_for_same_book_twice(self, name):
        self.collector.add_new_book(name)
